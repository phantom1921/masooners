...
class ProductCategoryController extends Controller
{
    ...
    public function store(StoreProductCategoryRequest $request)
    {
        ...
        if ($request->input('category_image', false)) {
            $productCategory->addMedia(storage_path('tmp/uploads/' . basename($request->input('category_image'))))->toMediaCollection('category_image');
        }
    ...
    public function update(UpdateProductCategoryRequest $request, ProductCategory $productCategory)
    {
        ...
        if ($request->input('category_image', false)) {
            if (!$productCategory->category_image || $request->input('category_image') !== $productCategory->category_image->file_name) {
                if ($productCategory->category_image) {
                    $productCategory->category_image->delete();
                }
                $productCategory->addMedia(storage_path('tmp/uploads/' . basename($request->input('category_image'))))->toMediaCollection('category_image');
            }
        } elseif ($productCategory->category_image) {
            $productCategory->category_image->delete();
        }
