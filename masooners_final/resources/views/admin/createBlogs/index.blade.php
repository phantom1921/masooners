...
@can('create_blog_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.create-blogs.create') }}">
                        ...
                        <tr data-entry-id="{{ $createBlog->id }}">
                            ...
                            <td>
                                @can('create_blog_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.create-blogs.show', $createBlog->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('create_blog_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.create-blogs.edit', $createBlog->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('create_blog_delete')
                                    <form action="{{ route('admin.create-blogs.destroy', $createBlog->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('create_blog_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.create-blogs.massDestroy') }}",
