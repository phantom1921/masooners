...
@can('pro_sub_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.pro-sub-categories.create') }}">
                        ...
                        <tr data-entry-id="{{ $proSubCategory->id }}">
                            ...
                            <td>
                                @can('pro_sub_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.pro-sub-categories.show', $proSubCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('pro_sub_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.pro-sub-categories.edit', $proSubCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('pro_sub_category_delete')
                                    <form action="{{ route('admin.pro-sub-categories.destroy', $proSubCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('pro_sub_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.pro-sub-categories.massDestroy') }}",
