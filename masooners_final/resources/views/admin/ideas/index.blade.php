...
@can('idea_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.ideas.create') }}">
                ...
                <tbody>
                    @foreach($ideas as $key => $idea)
                        <tr data-entry-id="{{ $idea->id }}">
                            ...
                            <td>
                                @can('idea_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.ideas.show', $idea->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('idea_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.ideas.edit', $idea->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('idea_delete')
                                    <form action="{{ route('admin.ideas.destroy', $idea->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('idea_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.ideas.massDestroy') }}",
