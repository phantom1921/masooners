...
@can('professional_detail_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.professional-details.create') }}">
                        ...
                        <tr data-entry-id="{{ $professionalDetail->id }}">
                            ...
                            <td>
                                @can('professional_detail_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.professional-details.show', $professionalDetail->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('professional_detail_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.professional-details.edit', $professionalDetail->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('professional_detail_delete')
                                    <form action="{{ route('admin.professional-details.destroy', $professionalDetail->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('professional_detail_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.professional-details.massDestroy') }}",
