                        ...
                        <tr data-entry-id="{{ $blogCategory->id }}">
                            ...
                            <td>
                                @can('blog_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.blog-categories.show', $blogCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('blog_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.blog-categories.edit', $blogCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('blog_category_delete')
                                    <form action="{{ route('admin.blog-categories.destroy', $blogCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
