...
@can('idea_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.idea-categories.create') }}">
            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-IdeaCategory">
                ...
                <tbody>
                    @foreach($ideaCategories as $key => $ideaCategory)
                        <tr data-entry-id="{{ $ideaCategory->title }}">
                            <td>
                            ...
                            <td>
                                @can('idea_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.idea-categories.show', $ideaCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('idea_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.idea-categories.edit', $ideaCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('idea_category_delete')
                                    <form action="{{ route('admin.idea-categories.destroy', $ideaCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('idea_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.idea-categories.massDestroy') }}",
