        ...
        <div class="col-md-12">
            @can('profile_comment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.profile-comments.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $profileComment->id }}">
                                        ...
                                        <td>
                                            @can('profile_comment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.profile-comments.show', $profileComment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('profile_comment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.profile-comments.edit', $profileComment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('profile_comment_delete')
                                                <form action="{{ route('frontend.profile-comments.destroy', $profileComment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('profile_comment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.profile-comments.massDestroy') }}",
