        ...
        <div class="col-md-12">
            @can('product_comment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.product-comments.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $productComment->id }}">
                                        ...
                                        <td>
                                            @can('product_comment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.product-comments.show', $productComment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('product_comment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.product-comments.edit', $productComment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('product_comment_delete')
                                                <form action="{{ route('frontend.product-comments.destroy', $productComment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('product_comment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.product-comments.massDestroy') }}",
