        ...
        <div class="col-md-12">
            @can('product_style_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.product-styles.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $productStyle->id }}">
                                        ...
                                        <td>
                                            @can('product_style_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.product-styles.show', $productStyle->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('product_style_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.product-styles.edit', $productStyle->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('product_style_delete')
                                                <form action="{{ route('frontend.product-styles.destroy', $productStyle->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('product_style_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.product-styles.massDestroy') }}",
