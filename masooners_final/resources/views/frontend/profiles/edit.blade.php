                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.profiles.update", [$profile->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="last_name" id="last_name" value="{{ old('last_name', $profile->last_name) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control ckeditor" name="about" id="about">{!! old('about', $profile->about) !!}</textarea>
                            @if($errors->has('about'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('about') }}
                                </div>
                        ...
                        <div class="form-group">
                            <label for="customerid">{{ trans('cruds.profile.fields.customerid') }}</label>
                            <input class="form-control" type="text" name="customerid" id="customerid" value="{{ old('customerid', $profile->customerid) }}">
                            @if($errors->has('customerid'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('customerid') }}
                                </div>
                            @endif
                            <span class="help-block">{{ trans('cruds.profile.fields.customerid_helper') }}</span>
                        </div>
                        <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.profiles.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $profile->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
