        ...
        <div class="col-md-12">
            @can('portfolio_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.portfolios.create') }}">
                            ...
                            <tbody>
                                @foreach($portfolios as $key => $portfolio)
                                    <tr data-entry-id="{{ $portfolio->id }}">
                                        ...
                                        <td>
                                            @can('portfolio_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.portfolios.show', $portfolio->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('portfolio_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.portfolios.edit', $portfolio->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('portfolio_delete')
                                                <form action="{{ route('frontend.portfolios.destroy', $portfolio->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('portfolio_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.portfolios.massDestroy') }}",
