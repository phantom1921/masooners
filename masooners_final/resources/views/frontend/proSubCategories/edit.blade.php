        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.proSubCategory.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.pro-sub-categories.update", [$proSubCategory->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="prof_category_id">{{ trans('cruds.proSubCategory.fields.prof_category') }}</label>
                            <select class="form-control select2" name="prof_category_id" id="prof_category_id" required>
                                @foreach($prof_categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('prof_category_id') ? old('prof_category_id') : $proSubCategory->prof_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $proSubCategory->name) }}" required>
                            @if($errors->has('name'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('name') }}
                                </div>
...
<script>
    ...
    url: '{{ route('frontend.pro-sub-categories.storeMedia') }}',
    maxFilesize: 2, // MB
